...
then
  ...
  export YTT_ingress__domain=${TAP_INGRESS_DOMAIN}
  ...
  git remote add origin https://gitea_admin:$GITEA_PASSWORD@gitea.${TAP_INGRESS_DOMAIN}/gitea_admin/$REPO_NAME.git
  git push -u origin main

  argocd login argocd-cli.${TAP_INGRESS_DOMAIN} --username admin --password $ARGOCD_PASSWORD
  argocd app set ${SESSION_NAMESPACE} --repo https://gitea.${TAP_INGRESS_DOMAIN}/gitea_admin/$REPO_NAME
fi