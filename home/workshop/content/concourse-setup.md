...
```dashboard:open-url
url: https://kubeapps.{{ ENV_TAP_INGRESS_DOMNAIN }}/#/c/default/ns/{{ session_namespace }}/apps
```
...
command: |-
  ...
  --data-value commonSecrets.harborDomain=harbor.{{ ENV_TAP_INGRESS_DOMNAIN }} \
  --data-value commonSecrets.harborPassword=$HARBOR_PASSWORD \
  --data-value commonSecrets.artifactoryDomain=artifactory.{{ ENV_TAP_INGRESS_DOMNAIN }} \
  ...
  --data-value commonSecrets.concourseHelperImage=harbor.{{ ENV_TAP_INGRESS_DOMNAIN }}/concourse/concourse-helper \
  ...
  --data-value petclinic.host=petclinic-{{ session_namespace }}.{{ ENV_TAP_INGRESS_DOMNAIN }} \
  --data-value petclinic.image=harbor.{{ ENV_TAP_INGRESS_DOMNAIN }}/{{ session_namespace }}/spring-petclinic \
...
```dashboard:open-url
url: https://concourse.{{ ENV_TAP_INGRESS_DOMNAIN }}/teams/{{ session_namespace }}/pipelines/spring-petclinic
```
...
```dashboard:open-url
url: https://petclinic-{{ session_namespace }}.{{ ENV_TAP_INGRESS_DOMNAIN }}
```
