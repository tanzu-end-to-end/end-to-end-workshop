...
```dashboard:open-url
url: https://kubeapps.{{ ENV_TAP_INGRESS_DOMAIN }}/#/c/default/ns/{{ session_namespace }}/apps
```
...
command: |-
  ...
  --data-value commonSecrets.harborDomain=harbor.{{ ENV_TAP_INGRESS_DOMAIN }} \
  --data-value commonSecrets.harborPassword=$HARBOR_PASSWORD \
  --data-value commonSecrets.artifactoryDomain=artifactory.{{ ENV_TAP_INGRESS_DOMAIN }} \
  ...
  --data-value commonSecrets.concourseHelperImage=harbor.{{ ENV_TAP_INGRESS_DOMAIN }}/concourse/concourse-helper \
  ...
  --data-value petclinic.host=petclinic-{{ session_namespace }}.{{ ENV_TAP_INGRESS_DOMAIN }} \
  --data-value petclinic.image=harbor.{{ ENV_TAP_INGRESS_DOMAIN }}/{{ session_namespace }}/spring-petclinic \
...
```dashboard:open-url
url: https://concourse.{{ ENV_TAP_INGRESS_DOMAIN }}/teams/{{ session_namespace }}/pipelines/spring-petclinic
```
...
```dashboard:open-url
url: https://petclinic-{{ session_namespace }}.{{ ENV_TAP_INGRESS_DOMAIN }}
```
